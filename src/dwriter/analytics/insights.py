...
class InsightGenerator:
    ...
    def __init__(self, engine: AnalyticsEngine) -> None:
        """Initialize the generator with an analytics engine."""
        self.engine = engine
    ...
    def _colorize(self, text: str) -> str:
        ...
        # This regex matches: (1) Rich tags OR (2) #tags OR
        # (3) &projects OR (4) numbers in parentheses
        pattern = r"(\[.*?\])|#(\w+)|&(\w+)|\((\d+)\)"
    ...
    def generate_insights(self) -> list[str]:
        ...
        if burnout > 0.7:
            raw_insights.append(
                "⚠️ [bold #f38ba8]Burnout Warning:[/] [#cdd6f4]You've been "
                "burning the candle at both ends. Try to sign off earlier "
                "tonight.[/]"
            )
        elif burnout > 0.4:
            raw_insights.append(
                "⏱️ [bold #f9e2af]Watch Your Pace:[/] [#cdd6f4]Late-night "
                "sessions are adding up. Consider grouping tasks to finish "
                "faster.[/]"
            )
        ...
        if added > 0:
            ...
            if completion_rate >= 0.8:
                raw_insights.append(
                    f"⚖️ [bold #a6e3a1]Great Follow-through![/] [#cdd6f4]You "
                    f"finished {done} out of {added} tasks. Keep this "
                    f"momentum going![/]"
                )
            elif completion_rate < 0.4:
                raw_insights.append(
                    f"📦 [bold #f9e2af]Backlog Alert:[/] [#cdd6f4]You added "
                    f"{added} tasks but only finished {done}. Maybe it's "
                    f"time to say 'no' to something new?[/]"
                )
        ...
        if switches > 4.0:
            raw_insights.append(
                f"🔄 [bold #f9e2af]Context Switcher:[/] [#cdd6f4]You're "
                f"juggling {switches:.1f} projects a day. Try focusing on "
                f"just one for a few hours.[/]"
            )
        ...
        if roi_data:
            ...
            if ratio > 3.0 and entries > 5:
                raw_insights.append(
                    f"🚧 [bold #fab387]Project Friction: "
                    f"&{highest_friction_proj}[/]. [#cdd6f4]Lots of notes "
                    f"({entries}) but few tasks completed ({todos}). Try "
                    f"breaking this down into smaller steps.[/]"
                )
        ...
        if total_pending > 0 and (dead / total_pending) > 0.3:
            raw_insights.append(
                f"🧹 [bold #f38ba8]Staleness Alert:[/] [#cdd6f4]{dead} "
                f"tasks have gone cold. Consider a cleanup or a fresh "
                f"start on them.[/]"
            )
        ...
        if deep_ratio < 20.0 and (deep_count + shallow_count) > 5:
            raw_insights.append(
                "🧘 [bold #a6e3a1]Focus Time Needed:[/] [#cdd6f4]Admin "
                "tasks are taking over. Schedule a deep-work session "
                "today.[/]"
            )
        elif deep_ratio > 50.0:
            raw_insights.append(
                "🔥 [bold #a6e3a1]High Focus:[/] [#cdd6f4]You are "
                "dedicating a significant portion of your time to deep "
                "work sessions.[/]"
            )
        ...
        if tag_velocity:
            ...
            raw_insights.append(
                f"🏷️ [bold #89b4fa]Active Focus:[/] [#cdd6f4]Recent "
                f"activity is concentrated in {tags_str}.[/]"
            )

        # Default summary if no specific alerts
        if not raw_insights:
            raw_insights.append(
                "✨ [bold #a6e3a1]Current Status:[/] [#cdd6f4]Your "
                "workload distribution appears consistent across active "
                "projects.[/]"
            )
    ...
    def generate_weekly_wrapup(self) -> list[str]:
        ...
        raw_insights.append(
            f"🎭 [bold #cba6f7]Your Archetype:[/] You were "
            f"[bold]{archetype}[/] this week."
        )

        # 2. Peak Velocity
        golden_hour = self.engine.get_golden_hour()
        raw_insights.append(
            f"⚡ [bold #f9e2af]Peak Velocity:[/] Your 'Golden Hour' "
            f"of highest focus was at {golden_hour}."
        )
        ...
        if big_rock_data:
            ...
            raw_insights.append(
                f"⛰️ [bold #89b4fa]The Big Rock:[/] &{proj} claimed "
                f"{pct:.0f}% of your bandwidth."
            )
        else:
            raw_insights.append(
                "⛰️ [bold #89b4fa]The Big Rock:[/] No single project "
                "dominated your week."
            )
        ...
        raw_insights.append(
            f"🚀 [bold #a6e3a1]Momentum:[/] You cleared "
            f"{current_cleared} tasks ({abs(pct_change)}% {trend} "
            f"than last week)."
        )
